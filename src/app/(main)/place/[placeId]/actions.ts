...
export async function getPlaceDetailsByIdAction(
  ...
  console.log(
    `getPlaceDetailsByIdAction: Fetching details for placeId: ${placeId}`,
  );

  try {
    const apiResponse = await goKyInstance
      .get(`maps/place/${placeId}`)
      .json<GoPlaceDetailApiResponse>();

    if (apiResponse.status === "OK" && apiResponse.result) {
      console.log(
        `getPlaceDetailsByIdAction: Successfully fetched details for ${apiResponse.result.name}`,
      );
      return { success: true, data: apiResponse.result };
    } else {
      console.error(
        `getPlaceDetailsByIdAction: API error for placeId ${placeId}. Status: ${apiResponse.status}, Message: ${apiResponse.error_message}`,
      );
      return {
        success: false,
        error: apiResponse.error_message || `API Error: ${apiResponse.status}`,
      };
    }
  } catch (error: any) {
    console.error(
      `getPlaceDetailsByIdAction: Exception for placeId ${placeId}:`,
      error,
    );

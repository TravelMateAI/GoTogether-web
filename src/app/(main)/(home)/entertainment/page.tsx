...
import { getEntertainmentPlacesAction } from "./actions";
  ...
  useEffect(() => {
    const fetchEntertainmentData = async () => {
      ...
      startTransition(async () => {
        const result = await getEntertainmentPlacesAction(locationToUse);
        if (result.success && result.data) {
          setPlaces(result.data);
        } else {
          setError(result.error || "Failed to fetch entertainment spots.");
          setPlaces([]);
        }
        setLoading(false);
      });
    ...
    <div className="flex-1 p-4 sm:p-6 bg-gray-100 dark:bg-slate-900 min-h-screen">
      <header className="mb-6 sm:mb-8">
        ...
        <p className="text-md sm:text-lg text-gray-600 dark:text-gray-400 mt-1">
          Find movie theaters, nightlife, bars, and other fun spots.
        </p>
      </header>
      ...
      {loading && (
        <div className="flex justify-center items-center py-10">
          ...
          <p className="ml-3 text-lg text-gray-700 dark:text-gray-300">Loading entertainment spots...</p>
        </div>
      )}
      ...
      {!loading && !error && places.length === 0 && (
        <p className="text-center text-gray-500 dark:text-gray-400 py-10 text-lg">
          No entertainment spots found for the current location.
        </p>
      )}

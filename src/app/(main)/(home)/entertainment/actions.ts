...
export async function getEntertainmentPlacesAction(
  location: string,
): Promise<GetEntertainmentPlacesResult> {
  ...
  console.log(
    `getEntertainmentPlacesAction: Fetching for location: ${location}, radius: ${radius}`,
  );
  ...
  if (result.success && result.data) {
    console.log(
      `getEntertainmentPlacesAction: Successfully fetched ${result.data.length} entertainment places.`,
    );
  } else if (!result.success) {
    console.error(
      `getEntertainmentPlacesAction: Error fetching entertainment places: ${result.error}`,
    );
  }

...
export async function getCulturePlacesAction(
  location: string,
): Promise<GetCulturePlacesResult> {
  ...
  console.log(
    `getCulturePlacesAction: Fetching for location: ${location}, radius: ${radius}`,
  );
  ...
  if (result.success && result.data) {
    console.log(
      `getCulturePlacesAction: Successfully fetched ${result.data.length} culture places.`,
    );
  } else if (!result.success) {
    console.error(
      `getCulturePlacesAction: Error fetching culture places: ${result.error}`,
    );
  }

...
import { getTopPicksPlacesAction } from "./actions";
  ...
  useEffect(() => {
    const fetchTopPicksData = async () => {
      ...
      startTransition(async () => {
        const result = await getTopPicksPlacesAction(locationToUse);
        if (result.success && result.data) {
          setPlaces(result.data);
        } else {
          setError(result.error || "Failed to fetch top picks.");
          setPlaces([]);
        }
        setLoading(false);
      });
    ...
    <div className="flex-1 p-4 sm:p-6 bg-gray-100 dark:bg-slate-900 min-h-screen">
      <header className="mb-6 sm:mb-8">
        <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 dark:text-gray-100">
          Top Picks & Attractions
        </h1>
        <p className="text-md sm:text-lg text-gray-600 dark:text-gray-400 mt-1">
          Explore highly-rated tourist attractions, landmarks, and parks.
        </p>
      </header>
      ...
      {loading && (
        <div className="flex justify-center items-center py-10">
          ...
          <p className="ml-3 text-lg text-gray-700 dark:text-gray-300">Loading top picks...</p>
        </div>
      )}
      ...
      {!loading && !error && places.length === 0 && (
        <p className="text-center text-gray-500 dark:text-gray-400 py-10 text-lg">
          No top picks found for the current location.
        </p>
      )}

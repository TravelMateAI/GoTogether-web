...
import { getStayPlacesAction } from "./actions";
  ...
  useEffect(() => {
    const fetchStayData = async () => {
      ...
      startTransition(async () => {
        const result = await getStayPlacesAction(locationToUse);
        if (result.success && result.data) {
          setPlaces(result.data);
        } else {
          setError(result.error || "Failed to fetch accommodation options.");
          setPlaces([]);
        }
        setLoading(false);
      });
    ...
    <div className="flex-1 p-4 sm:p-6 bg-gray-100 dark:bg-slate-900 min-h-screen">
      <header className="mb-6 sm:mb-8">
        ...
        <p className="text-md sm:text-lg text-gray-600 dark:text-gray-400 mt-1">
          Find hotels, resorts, campgrounds, and other lodging options.
        </p>
      </header>
      ...
      {loading && (
        <div className="flex justify-center items-center py-10">
          ...
          <p className="ml-3 text-lg text-gray-700 dark:text-gray-300">Loading places to stay...</p>
        </div>
      )}
      ...
      {!loading && !error && places.length === 0 && (
        <p className="text-center text-gray-500 dark:text-gray-400 py-10 text-lg">
          No accommodation options found for the current location.
        </p>
      )}
            ...
            <Link href={`/place/${place.place_id}`} key={place.place_id} legacyBehavior>
              <a className="bg-white dark:bg-slate-800 shadow-lg rounded-xl overflow-hidden hover:shadow-xl transition-shadow duration-300 ease-in-out block group">
                <div className="relative w-full h-48 sm:h-56">
                  ...
                  />
                  {place.opening_hours?.open_now !== undefined && ( // Note: 'open_now' might be less relevant for hotels but included if API provides it
                    <span className={`absolute top-2 right-2 px-2 py-1 text-xs font-semibold rounded-full ${
                      place.opening_hours.open_now
                        ? "bg-green-500/90 text-white"
                        : "bg-gray-500/90 text-white" // Using gray for 'Closed' for hotels as it's often N/A
                    }`}>
                      {place.opening_hours.open_now ? "Reception Open" : "Reception Hours May Vary"}
                    </span>
                  )}
                </div>
